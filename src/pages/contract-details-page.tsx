...
function badgeVariant(
  ...
  if (
    ["review", "approval", "authoring", "execution", "monitoring", "request", "storage"].includes(val)
  ) return "violet";
  return "blue";
}
...
function RichTextEditor({
  ...
  async function handleExport(format: "docx" | "pdf") {
    ...
    setExporting(format);
    try {
      await api.exportDocument(contractId, format, `${contractTitle}.${format}`);
    } catch (err) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-[#0F1320]">
      ...
      <div className="flex items-center justify-between border-b border-slate-50 bg-white px-5 py-1.5 dark:border-white/5 dark:bg-transparent">
        <span className="text-[11px] text-slate-400">Auto-saves every 2 seconds while you type</span>
        <span className="text-[11px] text-slate-400">Ctrl+B Bold · Ctrl+I Italic · Ctrl+U Underline</span>
      </div>
  ...
  if (!hasFile) return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-200 bg-slate-50 py-14 text-center dark:border-white/10 dark:bg-white/5">
      ...
      <p className="text-sm text-slate-400">
        ...
        <Link to="/upload" className="text-blue-500 underline hover:text-blue-600">Upload Pipeline</Link>{" "}
        to view it here
      </p>
    </div>
  );
    ...
    <div className="space-y-3">
      {(isPdf || isDocx) && (
        <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 dark:border-white/10 dark:bg-white/5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium text-blue-600 hover:text-blue-800 dark:hover:text-blue-400"
          >
      ...
      {showOrig && (isPdf || isDocx) && (
        <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-sm dark:border-white/10">
          ...
          {isDocx && <OriginalDocxViewer viewUrl={viewUrl} />}
        </div>
      )}
...
function SendApprovalPanel({
  contractId, workflowId, onDone, onClose,
}: {
  contractId: string;
  workflowId?: string;
  onDone: () => void;
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-500/20 dark:bg-blue-500/10">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onClose} className="text-blue-400 hover:text-blue-600 dark:hover:text-blue-200">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
            Approver User IDs{" "}
            <span className="font-normal normal-case text-slate-400">(comma-separated, optional)</span>
          </label>
          <input
            type="text"
            value={approverInput}
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" size="sm" className="rounded-xl border-slate-200 dark:border-white/10" onClick={onClose}>
          Cancel
        </Button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-white/10">
          ...
          <button onClick={onClose} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-white/10">
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          <button
            ...
            className={`flex w-full items-start gap-3 rounded-xl border p-4 text-left transition-all ${
              selected === "default"
                ...
                : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50 dark:border-white/10 dark:bg-transparent"
            }`}
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs leading-relaxed text-slate-500 dark:text-slate-400">{DEFAULT_STEP_DESCRIPTION}</p>
            </div>
            {selected === "default" && <Check className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" />}
          </button>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                    {t.steps.length} steps{t.description ? ` — ${t.description}` : ""}
                  </p>
                ...
                {selected === t.id && <Check className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" />}
              </button>
            ))
          )}
        </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4 dark:border-white/10">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" className="rounded-xl" onClick={() => onConfirm(chosenTemplate)}>
...
function WorkflowCard({
  workflows, contractId, onWorkflowCreated,
}: {
    ...
    <>
      ...
      <AppCard tone="soft">
        ...
        <div className="space-y-4">
          ...
          {!latest ? (
            <div className="space-y-3">
              <p className="text-sm text-slate-500 dark:text-slate-400">No workflow started yet.</p>
              <Button
                size="sm"
                variant="outline"
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <AppBadge variant={badgeVariant(latest.status)}>{fmt(latest.status)}</AppBadge>
              </div>
              ...
              <div className="flex items-center gap-1">
                {(latest.steps?.length
                  ? latest.steps
                  : Array.from({ length: 9 }, (_, i) => ({ step_number: i + 1, name: STEP_LABELS[i + 1], status: "pending" as const }))
                ).map((step) => {
                  ...
                  return (
                    <div
                      ...
                      title={step.name ?? STEP_LABELS[step.step_number] ?? `Step ${step.step_number}`}
              ...
              <p className="text-right text-[10px] text-slate-400">
                {latest.steps?.filter((s) => s.status === "completed").length ?? 0}{" "}
                / {latest.steps?.length ?? 9} steps completed
              </p>

              <Button asChild size="sm" variant="outline" className="w-full rounded-xl border-slate-200 dark:border-white/10">
...
function ApprovalCard({
  approvals, contractId, workflowId, currentUserClerkId, isAdminOrManager, onRefresh,
}: {
  approvals: Approval[];
  contractId: string;
  workflowId?: string;
  ...
  return (
    <AppCard tone="soft">
      ...
      <div className="space-y-4">
        {voteError && <p className="text-xs text-red-600 dark:text-red-300">{voteError}</p>}

        {!latest && !showPanel && (
          <div className="space-y-3">
            <p className="text-sm text-slate-500 dark:text-slate-400">No approval request yet.</p>
            <Button size="sm" className="w-full rounded-xl" onClick={() => setShowPanel(true)}>
            ...
            {voteCount && voteCount.total > 0 && (
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-xl bg-green-50 py-2 dark:bg-green-500/10">
                  ...
                  <p className="text-[11px] text-green-600 dark:text-green-300">Approved</p>
                </div>
                <div className="rounded-xl bg-red-50 py-2 dark:bg-red-500/10">
                  ...
                  <p className="text-[11px] text-red-600 dark:text-red-300">Rejected</p>
                </div>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {latest.approvers.map((a, i) => (
                    <div key={i} className="flex items-center justify-between rounded-xl border border-slate-100 px-3 py-2 dark:border-white/8">
                      <p className="max-w-[120px] truncate text-xs text-slate-600 dark:text-slate-300">{a.user_id}</p>
                      <AppBadge variant={badgeVariant(a.decision ?? "pending")} className="text-[10px]">
...
export default function ContractDetailsPage() {
  ...
  return (
    <AppShell
      ...
      actions={
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" asChild className="rounded-xl border-slate-200 dark:border-white/10">
    ...
    >
      ...
      {loading && (
        <div className="animate-pulse py-10 text-sm text-slate-400">Loading contract…</div>
      )}

      {!loading && !contract && (
        <p className="text-sm text-slate-500 dark:text-slate-400">Contract not found.</p>
      )}
      ...
      {contract && (
        <div className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 px-6 py-6 shadow-sm dark:border-white/10 dark:from-white/5 dark:to-transparent">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <AppBadge variant={badgeVariant(contract.status)}>{fmt(contract.status)}</AppBadge>
                  <AppBadge variant={badgeVariant(contract.workflow_stage)}>{fmt(contract.workflow_stage)}</AppBadge>
                  <AppBadge variant={badgeVariant(contract.risk_level)}>{fmt(contract.risk_level || "unrated")}</AppBadge>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                <div key={label} className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          <div className="grid gap-6 xl:grid-cols-[1.7fr_0.8fr]">
            <AppCard tone="soft">
              <div className="mb-5">
                <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Contract Details</h2>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {contract.tags?.length ? (
                      contract.tags.map((t, i) => (
                        <AppBadge key={i} variant="violet">{String(t).toUpperCase()}</AppBadge>
                      ))
                    ) : (
                    ...
                    <div className="space-y-2">
                      {contract.parties.map((p, i) => (
                        <div key={i} className="rounded-xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-white/5">
                          ...
                          {p.email && <p className="mt-0.5 text-xs text-slate-400">{p.email}</p>}
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-sm text-slate-500 dark:text-slate-400">No parties added yet.</p>
                  )}
                </div>
              </div>
            </AppCard>
            ...
            <div className="space-y-6">
              <WorkflowCard workflows={workflows} contractId={contract.id} onWorkflowCreated={load} />
                ...
                <AppCard tone="soft">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <AppBadge variant={badgeVariant(contract.ai_analysis.risk_level)}>
                    ...
                    <Button variant="outline" asChild size="sm" className="w-full rounded-xl border-slate-200 dark:border-white/10">
